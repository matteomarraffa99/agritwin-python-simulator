...
# TRUCCO ANTIBLOCCO: Generiamo le parentesi per i colori dinamicamente
def csi(codice):
...
class DigitalTwinAgricolo:
    ...
    def __init__(self):
        ...
        # Configurazione: Tempi, Capacita, Costi ed Emissioni
        self.modello = dict({
            "Grano": dict({
    ...
    def genera_meteo(self):
        ...
        scenari = tuple((
            tuple(("☀️  Clima Ideale", 1.0, 1.0, BG_VERDE + T_BIANCO)),
            tuple(("🔥 Ondata di Calore Anomala", 0.85, 1.15, BG_ROSSO + T_BIANCO)),
            tuple(("🌧️  Piogge Autunnali Intense", 0.95, 1.35, BG_GIALLO + T_NERO))
        ))
    ...
    def simula(self):
        ...
        # Generiamo il clima e lo salviamo (estraendo la tupla in 4 variabili)
    ...
    def stampa_dashboard(self):
        ...
        for chiave, dati in self.risultati.items():
            ...
            print("  ├─ ⚖️  Resa Netta  : " + BOLD + str(dati.get("tonnellate")) + " Ton" + RESET)
            print("  ├─ ⏱️  Tempi       : " + T_CIANO + str(dati.get("giorni")) + " Giorni" + RESET + " (Totale: " + str(dati.get("ore")) + " ore)")
            
            # Formattazione Numeri Europea (Punto per migliaia, virgola per decimali)
            str_costo = "{:,.2f}".format(dati.get("costo")).replace(",", "X").replace(".", ",").replace("X", ".")
            str_co2 = "{:,.1f}".format(dati.get("co2")).replace(",", "X").replace(".", ",").replace("X", ".")
            
            print("  ├─ 💶 Costi       : " + T_VERDE + "€ " + str_costo + RESET)
